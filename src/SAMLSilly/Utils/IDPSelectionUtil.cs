...
namespace SAMLSilly.Utils
{
    ...
    public class IdpSelectionUtil
    {
        ...
        public IdpSelectionUtil(ILoggerFactory logger)
        {
            if (logger == null) throw new ArgumentNullException("logger");
            this.logger = logger.CreateLogger<IdpSelectionUtil>();
        }
        /// <summary>
        ...
        public IdentityProvider RetrieveIDP(NameValueCollection allparams, NameValueCollection queryString, Saml2Configuration config, Action<string> redirectToSelection)
        {
            ...
            if (!string.IsNullOrEmpty(allparams[IdpChoiceParameterName])) {
                logger.LogDebug(TraceMessages.IdentityProviderRetreivedFromQueryString, allparams[IdpChoiceParameterName]);
            ...
            if (!string.IsNullOrEmpty(queryString["_saml_idp"])) {
                ...
                if (cdc.IsSet) {
                    ...
                    if (endPoint != null) {
                        logger.LogDebug(TraceMessages.IdentityProviderRetreivedFromCommonDomainCookie, cdc.PreferredIDP);
                        return endPoint;
                    }

                    logger.LogWarning(ErrorMessages.CommonDomainCookieIdentityProviderInvalid, cdc.PreferredIDP);
                }
            }
            ...
            if (config.IdentityProviders.Count == 1 && config.IdentityProviders[0].Metadata != null) {
                logger.LogDebug(TraceMessages.IdentityProviderRetreivedFromDefault, config.IdentityProviders[0].Name);
            ...
            if (defaultIDP != null) {
                logger.LogDebug(TraceMessages.IdentityProviderRetreivedFromDefault, defaultIDP.Id);
                return defaultIDP;
            }
            ...
            if (!string.IsNullOrEmpty(config.IdentityProviders.SelectionUrl)) {
                logger.LogDebug(TraceMessages.IdentityProviderRetreivedFromSelection, config.IdentityProviders.SelectionUrl);
